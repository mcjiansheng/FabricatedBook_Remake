...
- [ ] 统一前端布局、输入与渲染职责。
  ...
  - [ ] 地图顶栏的“卡牌/藏品”改为 Scene2D 控件或统一 UI 工厂创建的组件，不再由 `MapScreen` 手工绘制和命中测试。
  - [ ] 让布局常量、边距、字体层级、颜色、按钮状态从公共 theme/token 获取，避免散落在 Screen/Actor 中。
  ...
  - [ ] 为 `UiStyles` 等运行时生成的纹理建立统一 dispose 入口，明确 Screen 与全局主题各自拥有的资源。
...
- [ ] 重构战斗卡牌界面（参照《杀戮尖塔》的“手牌优先、目标明确、结果即时”结构）。
  ...
  - [ ] 支持以字体实际宽度和语义断行排版中文/英文/数字混合说明；长文本可缩排、关键词高亮或在详情面板展开，而不是静默省略关键信息。
  - [ ] 重整手牌的扇形/间距/悬停抬升/遮挡规则；手牌过多时仍应保证每张牌可辨认、可选中和可取消。
...
- [ ] 重构商店界面（参照《杀戮尖塔》的“商品分区 + 可比较成本 + 即时购买反馈”结构）。
  - [ ] 按卡牌、藏品、药水、服务（弃牌）分区；不要将全部商品压成一张三列文本表。
  - [ ] 卡牌商品复用卡牌展示组件；藏品和药水使用图标/名称/效果/价格卡片，使商品类型与价值在扫视时可区分。
  - [ ] 始终突出当前金币、商品价格、能否负担和购买后状态；金币不足和已购买要禁用并解释，而非只保留可点击的“购买”。
  - [ ] 完成弃牌服务：点击服务后进入牌组选择模态，展示价格、当前选择、确认/取消，并在成功后更新牌组与金币。
  - [ ] 为商品详情、购买成功、药水栏满、库存售罄设计统一反馈；购买刷新不得造成列表跳动或上下文丢失。

- [ ] 重构事件与奖励界面（参照《杀戮尖塔》的“叙事焦点 + 选项代价可读 + 结果过渡明确”结构）。
  - [ ] 事件页将标题、插图/场景、叙事文本、选项、代价和获得物分层；当前大段文字按钮不利于阅读、比较和后续增加条件。
  - [ ] 事件选项应明确显示金币、生命、卡牌、藏品、概率和前置条件；不可选项显示锁定原因，而不是直接隐藏关键路线信息。
  - [ ] 选择后展示独立结果状态，再由“继续前进”进入地图；结果页需要能承载新增/移除卡牌、获得藏品和数值变化。
  - [ ] 奖励页按金币、卡牌、药水、藏品分项展示，并确定“逐项领取/可跳过/满栏处理/提前离开”的最终规则；UI 不应替代尚未确认的玩法决策。

- [ ] 重构地图、总览与通用页面结构。
  - [ ] 地图节点除图标外增加可访问、已访问、路线、风险等级和节点类型的清晰区分；将层效果、当前路线目标和操作提示组织成稳定 HUD。
  - [ ] 卡牌与藏品总览分别进入对应标签页或页面，使用可滚动网格和详情查看；当前两个入口进入同一文本总览，信息架构不清晰。
