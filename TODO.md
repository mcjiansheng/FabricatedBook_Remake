...
- [ ] 建立构筑游戏所需的信息层级与交互反馈。
  ...
  - [ ] 为 hover、选中、拖拽目标、不可用、金币不足、已购买、药水栏满、操作成功/失败提供一致的视觉和文本反馈；不能只依赖状态更新后的静态文字。（已完成商店购买/弃牌/药水反馈和手牌不可打出态；拖拽目标、Buff 与奖励反馈待继续迁移。）
  - [ ] 用 tooltip 或详情面板承接 Buff、药水、藏品、关键词和长说明，避免把所有文字直接平铺到主界面。（已完成药水 hover Tooltip、战斗角色/敌人 Buff Tooltip 与总览详情栏；关键词说明仍待迁移。）
  - [ ] 明确键鼠操作：Esc、返回、取消拖拽、确认/跳过、滚动和模态阻断规则在所有界面保持一致。（已统一对局 Esc 菜单的背景输入阻断；其余规则继续收口。）
