...
- [x] 建立最小回归测试矩阵。（已新增 [doc/backend_regression_checklist.md](doc/backend_regression_checklist.md)，记录后端提交前命令、通过标志、失败处理和专项加跑范围。）
  ...
  - [x] 明确每次提交前至少运行哪些命令：`./gradlew test`、后端 CLI `selftest`、`seedtest`、`savetest`、`flowtest`、`routetest` 等。
- [x] 补齐 core 单元测试缺口。
  - [x] 伤害/格挡/状态伤害/预览已有部分测试。
  - [x] 药水伤害和格挡已有部分测试。
  - [x] 藏品状态伤害和累计胜利类效果已有部分测试。
  - [x] 补商店购买测试：卡牌、藏品、药水、金币不足、药水栏满退款、弃牌购买。（`ShopManagerTest` 已覆盖商品购买、重复购买拦截、金币不足不变式、药水满栏退款和弃牌计数/存档。）
  - [x] 补存档恢复测试：卡牌、藏品、药水、战斗中退出回到战前快照。（`GameRunStateSaveTest` 已覆盖卡牌升级态、藏品 ID、药水 ID、层数/金币/HP 恢复，以及战斗中保存回到战前快照。）
  - [x] 补藏品即时效果测试：加生命上限、立即金币、立即抽牌/给牌、负面藏品。（`DataRelicTest` 已覆盖拾取热水壶、古旧钱币、一份演讲稿和谦虚的即时效果。）
  - [x] 补地图生成可复现测试：同种子布局、连接、起终点、迷雾 Boss -> 抉择结构。（`LayerMapGraphTest` 已覆盖默认五层多 seed 的连线合法性、全节点可达、起终点类型和迷雾特殊 Boss 列。）
- [ ] 建立前端验收流程。
...
- [ ] 补复杂藏品专用实现。
  ...
  - [x] `"复仇者"`：伤害概率加成。（`DataRelic.modifyOutgoingDamage()` 已实现 1/3 触发 30% 增伤；`DataRelicTest` 使用确定性随机源覆盖触发/不触发边界，并确认预览不提前掷骰。）
  - [x] `"复仇者"`：第 3 层险路恶敌变为「迷失的守林人」。
  - [x] `"巴别塔"`：紧急作战额外敌人逻辑。（已下沉到 `EnemyEncounterResolver`，前端地图和后端 CLI 共用；`EnemyEncounterResolverTest` 覆盖额外敌人。）
...
- [ ] 补隐藏 Boss 和结局。
  ...
  - [x] 为隐藏路线补脚本化验收入口，避免只依赖人工点击。（后端 CLI 新增 `routetest`，覆盖门扉隐藏选项、巴别塔路线、第 5 层隐藏 Boss 和第一层回头结局。）
- [ ] 测试与文档。
  - [x] 为特殊藏品数值效果加单元测试或 CLI 回归。（`背叛`、`仇恨`、`集权`、`寡头`、`巴别塔`、隐藏 Boss 分流和 `复仇者` 概率伤害已有覆盖。）
  - [x] 为命运抉择、巴别塔可见条件、药水上限补核心单元测试。
  - [x] 为隐藏路线加种子/脚本化验收。（`EnemyEncounterResolverTest` 与 CLI `routetest` 已覆盖普通/隐藏 Boss 分流和隐藏路线条件。）
...
后端结构问题、修复顺序、影响范围和验证建议集中记录在 [doc/backend_todo.md](doc/backend_todo.md)。当前已完成战斗胜利回调、商店弃牌次数状态、战士卡牌来源统一，以及地图生成/连接规则 core 化；`LayerMapGraphTest` 已补齐默认五层多 seed 的地图可复现、连线合法、全节点可达、起终点和迷雾 Boss -> 抉择结构回归；节点进入与环境效果已下沉到 core，节点进入反馈已接入前端目标页面，战斗/未提交非战斗节点中途存档会回到节点入口前快照，事件选择、奖励节点事件、商店购买/删牌、安全屋结算和通用药水条丢弃等已提交非战斗进度会保留玩家变化并记录节点完成，最终 Boss 进入结局前会提交 active combat，结局显示时删除当前 run 存档。后端 CLI 商店节点已接入真实 `ShopManager`，支持商品购买、删牌和进度自动保存；奖励节点已和前端一致采用事件「好诗歪诗」映射；`flowtest` 会脚本化覆盖事件选择、奖励节点事件、商店商品购买、商店删牌、安全屋治疗和非战斗药水丢弃在提交后保存玩家变化并记录 completed node。下一步重点是继续做前端实际点击/截图回归、复杂玩法语义确认和职业内容完整化。

敌人遭遇选择已下沉到 `EnemyEncounterResolver`，前端地图和后端 CLI 共用普通/精英/Boss、巴别塔额外敌人、第 5 层普通/隐藏 Boss 分流规则；`EnemyEncounterResolverTest` 与 CLI `routetest` 会固定隐藏路线、门扉隐藏选项、幕后黑手分流和第一层回头结局。
