...
- [ ] 重构地图、总览与通用页面结构。
  ...
  - [ ] 标题、选角、安全屋、结局、失败/胜利弹窗采用同一面板与按钮语义，补足继续游戏失败、返回/放弃和确认提示。（已完成标题继续游戏失败反馈、职业选择面板、商店弃牌确认与战斗失败模态；安全屋、结局与胜利奖励继续迁移。）
  - [ ] 为视觉验收保留 `title`、`map`、`battle`、`shop`、`event` 调试入口，并补充固定数据/状态以复现复杂布局。（已补 `inventory`、`reward-potion` 与 `battle-defeat` 的复杂状态入口。）
