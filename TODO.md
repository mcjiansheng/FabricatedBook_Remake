...
事件系统已先让普通事件的名称、描述、选项展示和固定/简单随机/加权随机/占位藏品事件结果从 `events.json` 读取，并抽出 `EventResultResolver` 负责 JSON 结果转换；`EventRewardResolver` 已能把随机低阶/负面藏品占位展开为真实藏品；“生命回满”已改用显式 `fullHeal` 结果字段，不再依赖 `hpChange = 9999` 哨兵值，翅膀雕像摧毁、黏液世界放手、村庄板烧鸡腿堡已迁到 JSON 随机范围，投资已迁到 JSON 加权随机结果。后端 CLI `selftest` 会扫描 JSON 固定/随机事件结果字段、藏品引用和 Java executor 标记，`relic_nuke` / `relic_five_cards` / `relic_random_leq3` / `relic_curse_random` 已补为特殊奖励占位。卡牌奖励/核弹等特殊奖励 executor 和命运抉择条件分层仍待继续。
