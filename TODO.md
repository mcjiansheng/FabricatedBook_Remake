...
- [ ] 统一前端布局、输入与渲染职责。
  ...
  - [ ] 收口 `SpriteBatch` / `ShapeRenderer` 的切换边界；不要在单个 Actor 内频繁结束和重开 batch，以免后续动画、遮罩和批处理互相干扰。（奖励主面板、选牌和满药水选择已迁移到 Scene2D 模态；高频 `CardActor` 已迁为共享批渲染原语，角色 Actor 仍待后续组件化。）
