...
事件系统已先让普通事件的名称、描述、选项展示和部分固定事件结果从 `events.json` 读取，并抽出 `EventResultResolver` 负责固定 JSON 结果转换；“生命回满”已改用显式 `fullHeal` 结果字段，不再依赖 `hpChange = 9999` 哨兵值。随机事件结果和特殊事件 executor 分层仍待继续。
