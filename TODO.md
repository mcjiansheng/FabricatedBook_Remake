...
- [ ] 重构商店界面（参照《杀戮尖塔》的“商品分区 + 可比较成本 + 即时购买反馈”结构）。
  ...
  - [ ] 卡牌商品复用卡牌展示组件；藏品和药水使用图标/名称/效果/价格卡片，使商品类型与价值在扫视时可区分。（已完成两列名称/价格/说明/状态商品卡；专用图标与卡牌视觉组件待美术资源接入。）
...
- [ ] 重构地图、总览与通用页面结构。
  ...
  - [ ] 为视觉验收保留 `title`、`map`、`battle`、`shop`、`event` 调试入口，并补充固定数据/状态以复现复杂布局。（已补 `inventory`、`reward-potion`、`battle-defeat`、`character-select`、`ending` 与 `safe-house` 的复杂状态入口。）
