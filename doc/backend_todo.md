...
状态：部分修复。`DataLoader` 已新增 `loadEvents()`，普通事件的名称、描述和选项展示现在由 `events.json` 驱动；`events.json` 也开始承载固定事件结果字段（`outcomeDescription`、`goldChange`、`hpChange`、`fullHeal`、`relicId`、`outcome`）、简单随机范围字段（`goldChangeMin/Max`、`hpChangeMin/Max`）和加权随机结果列表（`randomOutcomes`）。`EventResultResolver` 会把可执行 JSON 选项转换为 `EventResult`，`EventRewardResolver` 会把 `relic_random_leq3` / `relic_curse_random` 等特殊奖励占位展开为真实藏品，并把 `relic_five_cards` 展开为 5 张真实可获得卡牌；前端事件页和后端 CLI 共用该奖励入口，`EventHandler` 只负责优先调度数据结果再回退 Java executor。“生命回满”已从 `hpChange = 9999` 哨兵值改为显式 `fullHeal` 字段，并由前端事件页和后端 CLI 共用；翅膀雕像摧毁、黏液世界放手、村庄板烧鸡腿堡已从 Java 随机分支迁到 JSON 随机范围，投资三个选项已迁到 JSON 加权随机结果，追猎逃跑与好诗歪诗的随机藏品占位结果也已迁到 JSON。命运抉择的描述、选项和巴别塔选项可见条件已迁入 JSON，使用 `randomPool: false` 避免进入普通随机事件池，并用 `executor: "java"` 明确复杂结果仍由 Java handler 处理。后端 CLI `selftest` 会扫描所有 JSON 固定/随机事件结果，确认字段可解析、随机范围顺序有效、加权 outcome 权重和描述有效、`relicId` 可创建、`fullHeal` 不与普通 `hpChange` 混用，并验证占位低阶/负面藏品可展开为真实藏品、五张牌奖励可展开为真实卡牌、命运抉择不会进入随机事件池。`relic_nuke`、`relic_five_cards`、`relic_random_leq3` 和 `relic_curse_random` 已补为特殊奖励路由 ID，避免事件奖励引用空 ID。核弹等特殊奖励还需要替换为正式 executor。测试覆盖普通事件文本来自 JSON、固定结果来自 JSON、随机范围结果来自 JSON、加权随机结果来自 JSON、占位藏品结果来自 JSON、占位藏品展开、五张牌展开、命运抉择展示数据/随机池边界、固定字段解析、显式回满、固定结果藏品 ID，以及命运抉择条件仍按玩家藏品判断。剩余工作：为核弹等特殊奖励设计明确 executor 分层。
